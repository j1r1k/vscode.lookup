...
- `npm run vscode:package` — Package into `dist/lookup.vsix`
...
- **`activate()`** — Registers three commands: `lookup.showPrompt`, `lookup.autocomplete`, `lookup.autocompleteBackwards`. Maintains shared mutable `state` object across commands.
...
- **Autocomplete commands** — Cycle through collected suffixes (forward/backward) by mutating `state.suffixes` and updating `quickPick.value`. Only active when context key `lookup.isQuickPickOpen` is true.
